...
Discovery-to-pipeline dry-run 会把 selected_for_deep_read 中的 A/B + High candidate 转成 metadata-only stub。stub 必须标记 `UNTRUSTED METADATA ONLY`、`body_is_untrusted`、未下载 PDF、未抓全文、不可支撑强结论。C/D 资料不得进入深读 stub，也不得用于生成强结论。
